...
public int GetNumberSubPixelsIn(Vector down_left, Vector up_right, int cur_depth) {
  ...
  int num_inter = 0;
  Vector vec1 = new Vector(down_left.x(), (up_right.y()+down_left.y())/2.0f); 
  Vector vec2 = new Vector((up_right.x()+down_left.x())/2.0f, up_right.y()); 
  num_inter += GetNumberSubPixelsIn(vec1, vec2, cur_depth+1);

  vec1 = new Vector((up_right.x() + down_left.x())/2.0f, (up_right.y()+down_left.y())/2.0f); 
  vec2 = new Vector(up_right.x(), up_right.y()); 
  num_inter += GetNumberSubPixelsIn(vec1, vec2, cur_depth+1);

  vec1 = new Vector( down_left.x(), down_left.y()); 
  vec2 = new Vector((up_right.x() + down_left.x())/2, (up_right.y()+down_left.y())/2.0f); 
  num_inter += GetNumberSubPixelsIn(vec1, vec2, cur_depth+1);

  vec1 = new Vector( (up_right.x() + down_left.x())/2, down_left.y()); 
  vec2 = new Vector(up_right.x(), (up_right.y()+down_left.y())/2.0f); 
  num_inter += GetNumberSubPixelsIn(vec1, vec2, cur_depth+1);
...
public float GetInterPercent(int i, int j) {
  float total = (float) Math.pow(4, max_depth);

  Vector down_left = new Vector( (float) i - 0.5f, (float) j - 0.5f);
  Vector up_right = new Vector( (float) i + 0.5f, (float) j + 0.5f);
...
public float cross_z(Vector a, Vector b) {
  return Vector.cross(a, b, null).z();
}
public int norm_sign(float f) {
  return f < 0 ? -1 : 1;
}

public boolean IsPointInsideTriangle(Vector p, Vector a, Vector b, Vector c) { 
...
public int GetColor(Vector p, Vector a, Vector b, Vector c) {
  ...
  int to_return = color(255.0f*red_level, 255.0f*green_level, 255.0f * blue_level, 150);
  return to_return;
}
  ...
  for (int i=low_x-1; i <= gre_x +1; i++) {
    for (int j=low_y-1; j<= gre_y + 1; j++) {
      ...
      if (opacity > 0) {
        ...
        int c = GetColor(p, v1_node, v2_node, v3_node);
